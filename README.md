...
- **Product screenshots** (`static/screenshots/anketa-preview-{light,dark}.png`) are real, not mockups — captured with Playwright against the actual running app (`encrypted1on1/frontend`, real dev backend, two real accounts with real X25519/argon2id crypto), then trimmed to one side-card's Mood/Feelings section for a tight, above-the-fold crop. Real example email addresses used during capture (`alex.chen.<timestamp>@example.com`) were swapped for clean display names (`alex.chen@example.com`) via a DOM text-node replacement _before_ the screenshot — a presentation-only fix, no real data involved either way since these are throwaway dev accounts. The hero (`+page.svelte`) ships both a light and a dark PNG and toggles between them with the exact same `[data-theme]`/`prefers-color-scheme` cascade `tokens.css` itself uses, so the screenshot always matches the page's own current theme. Not yet automated/scripted for reproducibility — if the app's UI changes enough to make these stale, they need to be recaptured by hand the same way.
...
- **ToS and Privacy Policy are English-only, deliberately** — every locale falls back to the same English text (`getLegalContent()` in `src/lib/content/index.ts`) rather than a translation of legal text that hasn't been reviewed yet. The rest of the site (`src/lib/content/{ru,lv,es,de,fr}.ts`) is still fully translated.
- **Marketing-copy translations (ru/lv/es/de/fr) were written by Claude, not reviewed by native speakers yet.** Structurally complete and parallel to the English source (same keys, same sections), but worth a native-speaker pass before this is genuinely public-facing.
